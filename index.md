...
9. <span id="核脉冲推进器">**核脉冲推进器**</span>(用原子弹或核弹的爆炸提供脉冲推力)([Wikipedia](https://en.wikipedia.org/wiki/Nuclear_pulse_propulsion))
   1. <span id="Orion">**猎户座计划(Orion)**</span>(在船尾投下核弹，将抛出的圆盘等离子化推动飞船，经缓冲后脉冲推进，*4383s(裂变)<I<sub>sp</sub><12000s(聚变)*)([Atomic Rockets](http://www.projectrho.com/public_html/rocket/enginelist3.php#boomboom)\|[Wikipedia](https://en.wikipedia.org/wiki/Project_Orion_(nuclear_propulsion)))
...
## 3. [材料合成](./Documentation/Manufacture)
...
## 5. 文明发展路径和[宇宙工程](./Documentation/CosmicEngineering)
...
## 6. [宇宙作战理论](./Documentation/CosmicCombat)
...
3. **物质武器**(利用发射弹头造成伤害)
   1. **动力方式**
      1. **核火炮**(类似传统的火炮，但由微型核爆炸推动弹头。通常用于核聚变推进器还未小型化时，其推进原理类似[猎户座计划](#Orion)通过等离子化的物质膨胀推进)
         1. **核机炮**(通过多管旋转机炮连发设计和足够迅速冷却火炮的散热板使得火炮可以连发，形成弹幕)

      2. **电磁轨道炮**
         1. **电磁机炮**(通过连发设计使得火炮可以连发，无需多管，但增加了飞船反应堆和冷却系统的压力，形成弹幕)

      3. **导弹**(携带自身动力装置和燃料，不依赖飞船发射设施)

   2. **制导方式**
      1. **自动调姿变轨**(弹头携带微型推进器(通常是微型电推或者微型核推)，少量燃料，传感器和弹道计算机，在飞向目标时进行中段和末端修正)
      2. **智能电磁场制导**(一种末端制导手段，通过特定电磁场抵消电磁偏导力场的偏转效果，从而提高准确度)
   3. **能量类型**
      1. **核聚变/核裂变**(利用核能进行推进或杀伤，或是产生大量中子产生大量辐射)
      2. **正反物质湮灭**(利用正反物质进行推进或杀伤)
      3. **霍金辐射黑洞**(利用短寿命[微型黑洞](#微型黑洞)的霍金辐射以[微型黑洞火箭推进器](#微型黑洞火箭推进器)的原理产生推力进行推进，或在接近敌舰时使[黑洞容器](#黑洞容器)分离，穿入敌舰内部产生大量能量进行杀伤)
   4. **弹头类型**
      1. **高爆弹(HE)**(杀伤类型为微型爆炸，通过碰撞船体瞬间触发引信在船体表面触发爆炸)
      2. **辐射弹(R)**(杀伤类型为强辐射，通过碰撞船体瞬间触发引信在船体表面产生大量辐射)
      3. **动能穿甲弹(AP)**(利用弹丸的动能造成伤害，通常用于点防御)
      4. **穿甲高爆弹(APHE)**(杀伤类型为微型爆炸，碰撞船体后触发引信在船体内部触发爆炸)
      5. **穿甲辐射弹(APR)**(杀伤类型为强辐射，碰撞船体后触发引信在船体内部产生大量辐射)
      6. **近炸辐射弹(R-VT)**(杀伤类型为强辐射，在靠近敌舰时触发引信在船体外部产生大量辐射)
      7. **微型黑洞(BH)**(杀伤类型为[微型黑洞](#微型黑洞)，处于[黑洞容器](#黑洞容器)中，在接近敌舰时使[黑洞容器](#黑洞容器)分离，穿入敌舰内部产生大量能量进行杀伤)
      8. **静电示踪弹**(简单的带有特定电荷和传感器的弹头，用于探测敌方飞船偏导力场的作用效果从而获得偏导力场的情报，提高等离子/粒子束武器和动能武器的有效性)
      9. **线圈示踪弹**(简单的带有特定通电超导线圈和传感器的弹头，用于探测敌方飞船电磁偏导力场的作用效果从而获得偏导力场的情报，提高等离子/粒子束武器和动能武器的有效性)
4. ~~**等离子物质抛射弹**(包裹有特定物质的核弹/反物质弹，产生大量飞溅的等离子体从而对周围的飞船造成伤害)~~
5. **粒子束武器**(质子束、中子束、α粒子束、电子束)
6. **等离子束线圈炮**(一个修改过的火箭推进器，用能量反应后聚合高温等离子体喷流进行杀伤，射程较短)
7. **等离子脉冲炮**(一个修改过的脉冲火箭推进器，用能量反应后聚合高温等离子体脉冲进行瞬时杀伤，射程较短)
8. **近距μ子束炮**(一种特殊的粒子束武器，目的在于命中装有惯性约束核聚变推进器飞船的燃料库，引发μ子催化聚变，从而在内部对敌舰产生大量伤害)
...
### 1. [工程设想](./Documentation/Engineering)
...
8. **土星滑翔机**(一种假想的可以稳定在气态行星低层大气低速巡航的太空飞机，可以帮助拍摄土星夜间极其壮观的图片)

9. **可扩展性医用机械外骨骼**(一种用于医疗辅助的微型机械外骨骼)

10. **生物分子电力供能器**(一种假想的用电能维持人体生物循环的设备)

11. **通用纳米技术核心模块**(一种假想的高级文明用来组成科技设备的基本单位，可以自由地进行编程和重组从而从而将一种科技转为另一种科技)

12. **微型惰性气体托卡马克模拟装置**(一个装有电磁线圈的托卡马克模型，内部充有惰性气体，在运行时可以观察到惰性气体通电发光和被约束的过程)

13. **小型飞船缆绳式人工重力解决方案**(用缆绳和可分离配重块最大化飞船居住舱和重心的距离，从而使小型飞船可以实现人工重力)

14. **相对论性μ子存储线圈**(通过将μ子加速到接近光速从而延长其衰变所需要的时间，进而实现μ子长时间存储从而实现μ子催化聚变)

15. **根式单向加密**(由考试用991计算器猜答案的过程可知，如果对一个数进行含有加减乘除和开根的运算，很难猜出原数是什么，因此这个原理可用于加密)

16. **电磁雨伞**(一个神奇的没有伞面的微型雨伞，原理类似[巴萨德冲压发动机](#巴萨德冲压发动机)，可以发射激光等离子化雨滴并用磁场将其偏转，从而阻止伞下的人淋到雨)

17. **蚊虫点防御**(运用三维成像系统和大功率激光，从而高效击落那些尝试在你大腿上进行俯冲轰炸的蚊子)

18. **开源物理可视化框架(Open Physics Visualization Framework, OPVF)**(一个开源物理引擎计划，旨在渲染出符合物理的图像和结构，从而用于科幻作品)

### 2. [规律和定律](./Documentation/Law)
...
12. **光子气体说**(光子，若处于反射率达100%的理想容器中，表现得如同理想气体)
